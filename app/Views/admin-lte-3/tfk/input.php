...
<?php $months = ['jan'=>'Januari','feb'=>'Februari','mar'=>'Maret','apr'=>'April','mei'=>'Mei','jun'=>'Juni','jul'=>'Juli','ags'=>'Agustus','sep'=>'September','okt'=>'Oktober','nov'=>'November','des'=>'Desember']; $currentYear = (int)($_GET['year'] ?? date('Y')); ?>
<div class="card">
	<div class="card-header d-flex justify-content-between align-items-center">
		<h3 class="card-title">Target Fisik & Keuangan - Input (Tahun <?= $currentYear ?>)</h3>
		<div>
			<a href="<?= base_url('tfk?year=' . $currentYear) ?>" class="btn btn-sm btn-secondary">Data</a>
		</div>
	</div>
	<div class="card-body">
		<?php if (isset($master)): ?>
		<div class="table-responsive mt-2">
			<table class="table table-bordered mb-0" id="tfkTable" data-master-id="<?= (int)$master['id'] ?>" data-year="<?= $currentYear ?>">
				<thead>
					<tr>
				...
				<tbody>
					<?php
					$map = $details;
					function getValY($map,$bulan){ return isset($map[$bulan]) ? $map[$bulan] : []; }
					?>
					<tr>
		...
		<?php else: ?>
		<div class="alert alert-info mb-0">Pilih master dari menu Data terlebih dahulu.</div>
		<?php endif; ?>
	</div>
</div>
