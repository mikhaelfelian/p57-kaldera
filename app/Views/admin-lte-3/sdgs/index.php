...
<div class="row">
    <div class="col-md-12">
        <div class="card rounded-0">
            ...
            <div class="card-body">
                <?= form_open_multipart('sdgs/store', ['id' => 'sdgsForm', 'class' => 'mb-3']) ?>
                    <?= csrf_field() ?>
                    <div class="form-row align-items-end">
                        ...
                        <div class="form-group col-md-2">
                            <label for="month">Triwulan</label>
                            <select id="month" name="month" class="form-control rounded-0" required>
                                <?php for ($m = 1; $m <= 4; $m++): $v = str_pad($m, 2, '0', STR_PAD_LEFT); ?>
                                    <option value="<?= $v ?>">Triwulan <?= $m ?></option>
                                <?php endfor; ?>
                            </select>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="uraian">Uraian</label>
                            <input type="text" id="uraian" name="uraian" class="form-control rounded-0" placeholder="Monitoring Progres SDG’s" required>
                        </div>
                        <div class="form-group col-md-2">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped rounded-0">
                        <thead class="bg-primary text-white">
                            <tr>
                                <th class="text-center" style="width:10%">Triwulan</th>
                        ...
                        <tbody>
                            <?php if (!empty($items)): foreach ($items as $row): ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a class="btn btn-sm btn-primary rounded-0" href="<?= base_url('sdgs/preview/' . $row['id']) ?>" target="_blank" title="Preview">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a class="btn btn-sm btn-info rounded-0" href="<?= base_url('sdgs/download/' . $row['id']) ?>" title="Cetak / Download">
