...
<div class="row">
    <div class="col-md-9">
        <div class="card rounded-0">
            ...
            <div class="card-body">
                <?= form_open_multipart('gender/store', ['id' => 'genderForm', 'class' => 'mb-3']) ?>
                    <?= csrf_field() ?>
                        ...
                        <div class="form-group col-md-2">
                            ...
                            <select id="month" name="month" class="form-control rounded-0" required>
                                <?php for ($m = 1; $m <= 12; $m++): $v = str_pad($m, 2, '0', STR_PAD_LEFT); ?>
                                    <option value="<?= $v ?>" <?= (date('m') === $v ? 'selected' : '') ?>><?= bulan_ke_str($m) ?></option>
                                <?php endfor; ?>
                            </select>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="uraian">Uraian</label>
                            <input type="text" id="uraian" name="uraian" class="form-control rounded-0" placeholder="Monitoring Progres Gender" required>
                        </div>
                        <div class="form-group col-md-4">
                            <label for="fupload">Upload Data (PDF/Excel)</label>
                    ...
                    <table class="table table-bordered table-striped rounded-0">
                        <thead class="bg-primary text-white">
                            <tr>
                                ...
                                <th style="width:20%" class="text-center">Upload Data</th>
                        ...
                        <tbody>
                            <?php if (!empty($items)): ?>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a class="btn btn-sm btn-primary rounded-0" href="<?= base_url('gender/preview/' . $row['id']) ?>" target="_blank" title="Preview">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a class="btn btn-sm btn-info rounded-0" href="<?= base_url('gender/download/' . $row['id']) ?>" title="Cetak / Download">
